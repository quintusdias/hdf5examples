...
szip compression.  The program first checks if szip compression is
available, then if it is it writes integers to a dataset using gzip,
then closes the file.  Next, it reopens the file, reads back the
data, and outputs the type of compression and the maximum value in
...
def run():
    ...
    # an optional part of the hdf5 library.  if not
    if not h5py.h5z.filter_avail(h5py.h5z.FILTER_SZIP):
        raise RuntimeError("Szip filter not available.")

    filter_info = h5py.h5z.get_filter_info(h5py.h5z.FILTER_SZIP)
    if ((filter_info & h5py.h5z.FILTER_CONFIG_ENCODE_ENABLED) & 
        (filter_info & h5py.h5z.FILTER_CONFIG_DECODE_ENABLED)):
        msg = "Szip filter not available for encoding and decoding."
    ...
    # Have to hard-code the flags values.  Just deal with it.
    H5_SZIP_NN_OPTION_MASK = 32
    dcpl.set_szip(H5_SZIP_NN_OPTION_MASK, 8)
