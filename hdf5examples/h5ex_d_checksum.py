...
def run():
    ...
    # Check if the fletcher32 filter is available and can be used for both
    # encoding and decoding.
    if not h5py.h5z.filter_avail(h5py.h5z.FILTER_FLETCHER32):
        raise RuntimeError("Fletcher32 filter not available.")

    filter_info = h5py.h5z.get_filter_info(h5py.h5z.FILTER_FLETCHER32)
    if ((filter_info & h5py.h5z.FILTER_CONFIG_ENCODE_ENABLED) & 
        (filter_info & h5py.h5z.FILTER_CONFIG_DECODE_ENABLED)):
        msg = "Fletcher32 filter not available for encoding and decoding."
    ...
    dset = h5py.h5d.create(fid, DATASET, h5py.h5t.STD_I32LE, 
    ...
    if filter_type != h5py.h5z.FILTER_FLETCHER32:
